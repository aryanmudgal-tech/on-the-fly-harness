...
| Model-agnostic harness | Manus: be the boat riding the model tide, never train [32] | Cursor: rewired its long-running-agent harness around GPT-5.1/5.2 for instruction following, then shipped its own model, Composer 2 (Mar 2026) (via secondary) [40][39] | Coupling has academic support: "harness design and post-training cannot be treated as separable design choices" [46][47] |
