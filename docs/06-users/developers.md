...
Sourcing note: the proxy blocked stackoverflow.co/.blog, jetbrains.com, dora.dev, metr.org, anthropic.com, github.blog, faros.ai, cursor.com, openai.com and every.to; claims from them rest on two or more search excerpts or secondary write-ups and are marked "(via secondary)". Read directly: Claude Code documentation on code.claude.com, README files on raw.githubusercontent.com (Symphony, Gas Town, the Ralph Wiggum plugin, METR's repository), Google Cloud's DORA announcement, and the anthropics/claude-code issue tracker via the GitHub API (reaction counts as of 2026-09-08). Revenue figures and product dates are cross-referenced to sibling documents, not re-verified.
...
- Agents went from minority to majority in a year: 31% of developers used agents in mid-2025 [2]; JetBrains reports 90% of professional developers using coding agents at least weekly by May–July 2026 (single excerpt; unverified) [8].
- Measured productivity is contested. METR's July 2025 trial found experienced open-source developers 19% slower while believing they were 20% faster; its February 2026 follow-up could not produce a reliable estimate, partly because developers ran several agents at once (via secondary) [11][12]. Anthropic's engineers self-report roughly 50% gains (vendor study) [13].
- The bottleneck moved to review: 98% more PRs merged, review time up 91%, PR size up 154% in Faros telemetry (vendor, via secondary) [15]; DORA finds AI now helps throughput but still hurts stability [10].
...
- The terminal is becoming an engine, not disappearing: one engine behind terminal, IDE, desktop, web, phone and chat [17], and every lab and most independents now ship an "agent manager" that runs many CLI sessions in git worktrees [26][39][42].
- For the thesis: "the CLI cannot be the default surface" is supported; "developers will leave the terminal" is not. Developers are leaving *watching* the terminal.
...
| --- | --- | --- | --- |
| Stack Overflow 2025 (Jul 2025) [1][2][3][6] | Annual survey | 84% use or plan to use AI; 46% distrust accuracy vs 33% trust, 3% highly trust; agents 31% use, 17% plan, 38% never; 66% "almost right"; 45% say debugging AI code takes longer; trust in accuracy reported down from 40% to 29% | Via secondary; 33% and 29% may be different questions |
...
| JetBrains 2025 (Oct 2025) [7] | 24,534 developers | 85% use AI regularly; 62% rely on an assistant, agent or AI editor; 44% say it is integrated into their workflow | Via secondary |
| JetBrains agents post (Aug 2026) [8] | Professionals, May–Jul 2026 | 90% use coding agents weekly, 68% daily | One excerpt; unverified |
...
| METR follow-up (Feb 2026) [12] | 57 developers, 143 repos, 800+ tasks | Returning developers 18% slower (CI −38% to +9%); new developers 4% slower (−15% to +9%); data judged unreliable | Developers refused to work without AI; time on task unmeasurable with several agents running |
...
| GitHub Octoverse (Oct 2025) [14] | Platform data | 180M+ developers; 43.2M PRs merged per month (+23%); 1.1M public repos import an LLM SDK (+178%); ~80% of new developers use Copilot in week one | Via secondary |
...
Anthropic's guide: "Claude's context window fills up fast, and performance degrades as it fills"; "the context window is the most important resource to manage" [18]. The remedies are manual hygiene: `/clear` between tasks, `/compact` with instructions, subagents for exploration, `/btw` for side questions, and a CLAUDE.md under 200 lines because bloated files "cause Claude to ignore your actual instructions" [18][20]. The tracker shows the failures: "/compact causes Claude Code to ignore CLAUDE.md" (Jul 2025), a request to expose remaining context (May 2025), a `--no-auto-compact` request (Aug 2025, 47 reactions), and May–June 2026 reports of auto-compact never firing at 100% [33]. Opus 5 brought a 1M-token window in July 2026, and the docs still describe degradation as it fills [32][18].
...
**Pull-request loops.** Auto-fix subscribes a session to a PR: "when a check fails or a reviewer leaves a comment, Claude investigates and pushes a fix if one is clear", asking on ambiguous requests [27]. Code Review posts severity-tagged inline findings and collects thumbs to tune the reviewer [29]. Symphony "turns project work into isolated, autonomous implementation runs, allowing teams to manage work instead of supervising coding agents", watching a Linear board and spawning one run per issue (repo created Feb 26, 2026; ~27k stars) [35]. The PR is now the agent's unit of accountability; the docs warn that an agent's reply can trigger comment-driven deploy automation [27].
...
2. **The permission model is better than the terminal's, not merely hidden.** Classifier plus sandbox plus organisation policy has to make "what can this fleet touch" legible [19].
