...
- Sandboxing is a commodity. Firecracker boots a microVM in under 125 ms with under 5 MiB overhead [10]; E2B, Modal, Cloudflare, Apple and Anthropic all ship isolation, and Anthropic lists eleven partner runtimes for self-hosted Managed Agents sandboxes [23]. Daytona, the most-starred sandbox project (71.8k stars), moved core development to a private codebase in June 2026 [5]. Isolation is not a moat.
...
- The defenses that work are harness-level: OS sandboxes, egress proxies that hold credentials outside the boundary, and a second model that reviews actions. Anthropic reports sandboxing cut permission prompts by 84% (vendor, via secondary) [107]; its auto-mode classifier is now the starting permission mode on Pro, Max and Team plans [62].
- Approval fatigue is measured: users approved 93% of prompts; humans caught 13.6% of dangerous commands versus 89% for the classifier (vendor figures, via secondary) [65]. Everyone converged on "replace the human prompt with a machine judge plus a hard boundary."
...
| gVisor (Google) | User-space kernel (`runsc`) [12] | Container-like | Apache-2.0 | GKE, Cloud Run [12]; simple syscalls about 2x slower, heavy file I/O "10-200x slower" per Anthropic's guide [19] |
...
| Modal Sandboxes | gVisor (secondary) [4] | Sub-second | Proprietary | Lovable and Quora as customers (vendor via secondary); about $0.047 per vCPU-hour (secondary) [4] |
...
| Cloudflare | One container per sandbox [7]; Dynamic Workers on V8 isolates, under 5 ms (secondary) [8] | ms | SDK "Beta," Apache-2.0 [7]; Dynamic Workers open beta Mar 2026 [8] | $0.002 per unique Worker per day, waived in beta (secondary) [9] |
| Apple `container` | One lightweight VM per Linux container [14] | "sub-second" (secondary) [15] | Apache-2.0; macOS 26; 1.0.0 on the project's first anniversary, Jun 2026 [14][15] | Adds `container machine`, a persistent Linux environment [14] |
...
Two patterns recur in every mature option. **Credentials live outside the boundary:** Claude Code on the web keeps GitHub tokens in a proxy that swaps a scoped credential for the real one [20]; the Bash sandbox can show commands a sentinel value and inject the real secret at the proxy, re-signing AWS SigV4 requests [17]; the Agent SDK guide prescribes the same proxy for any deployment [19]. **Isolation is a ladder, not a switch:** Claude Code documents six rungs from sandboxed Bash to hosted VM and states that the per-command sandbox "is not sufficient for fully unattended runs" and that `--dangerously-skip-permissions` belongs inside a container, VM or the runtime [18].
...
- *Anthropic.* `computer_toolset_20260801` works from screenshots, a `zoom` action and 17 member tools, costing about 4,500 input tokens per request [27][25]. `browser_toolset_20260801` (31 member tools) reads pages "through its structure (the accessibility tree, elements, forms, and tabs)" and through pixels; the browser "runs in your application's environment, not Anthropic's"; overhead about 6,600 tokens [28][25]. Claude Code drives native macOS apps in a research preview limited to Pro and Max, with per-app approval, a machine-wide lock, and the terminal excluded from screenshots so on-screen prompts cannot feed back into the model [29]. Its docs order the options by precision: MCP, then Bash, then Chrome, then screen control, "the broadest and slowest" [29].
...
**Benchmarks.** OSWorld's human baseline is 72.36% (secondary) [95]; OSWorld-Verified landed 28 Jul 2025 [92]. Vendor-reported 2026 scores exceed the human line: Sonnet 5 at 81.2%, Opus 4.8 at 83.4% (secondary) [94]; an aggregator lists 86.1% for a Qwen model (secondary, unverified) [95]. The official leaderboard was unreachable from this session.
...
- *Context editing.* `clear_tool_uses_20250919` clears the oldest tool results past 100,000 tokens, keeping the last three; `clear_thinking_20251015` manages reasoning blocks [55]. At the Sonnet 4.5 launch (Sep 2025) Anthropic reported memory plus context editing improved agentic search by 39%, editing alone by 29%, with 84% fewer tokens over 100 turns (vendor, secondary) [57].
...
| 23 Jan to 2 Feb 2026 | OpenClaw (ex Clawdbot, Moltbot) | Censys found 21,639 gateways on the public internet on 31 Jan (secondary); exposed instances leaked Anthropic keys, Telegram and Slack tokens and chat histories; GitHub advisory CVE-2026-25253, "1-Click RCE via Authentication Token Exfiltration From gatewayUrl," published 2 Feb 2026, severity High (CVSS 8.8 via secondary) [74][76][77] |
| Feb 2026 | OpenClaw skills registry | "ClawHavoc": 341 malicious skills, about 12% of ClawHub (secondary, unverified) [78] |
...
|---|---|---|
| Terminal-Bench 2.0 | 89 validated terminal tasks via Harbor; Nov 2025 [86][84] | Official leaderboard blocked here; aggregators list 91.9% and 65.9% for the same model family (secondary, unreliable) [112] |
| SWE-bench Verified | 500 issues validated with OpenAI, Aug 2024 [87] | Self-reported 80 to 97% on vendor scaffolds; contamination audits find solution leakage; OpenAI reportedly stopped reporting it in early 2026 (secondary, unverified) [91] |
| SWE-bench Pro (Scale) | Harder, partly private; SWE-agent scaffold [88] | Standardized public leaders 59.1 to 61.5% versus vendor-reported 80 to 81%; "most pages quoting a score never say which one they mean" (secondary) [90] |
...
| tau2 / tau3-bench | Customer-service agents against a simulated user and policy; pass^k reliability | Telecom leaders about 99% (likely avg@k, secondary) [97]; tau3 added full-duplex voice and a retrieval domain, Jul 2026 [96] |
...
- Auto mode launch date: secondary sources give 24 Mar 2026 and 10 Jul 2026; the switch to default on 14 Aug 2026 is also secondary [109][110]. Anthropic's blog and engineering posts were unreachable.
- The 93% approval rate, 13.6% versus 89% catch rates, "800 commands a human had approved," and 84% prompt reduction are Anthropic figures quoted by third parties [65][107][109].
...
- Apple `container` 1.0.0: the releases page rendered inconsistent years; the "container is one year old" note implies June 2026, matching secondary reports of 9 Jun 2026 [14][15].
...
- OpenAI stopping SWE-bench Verified reporting after an internal audit is a single secondary claim [91].
- Terminal-Bench 2.0 and OSWorld-Verified leaderboards were blocked; all 2026 scores come from aggregators or vendor posts and conflict [112][94][95].
...
- Mem0's 2025 paper claims were not re-verified; only the April 2026 README numbers are cited [45].
