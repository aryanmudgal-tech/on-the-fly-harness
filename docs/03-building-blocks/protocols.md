...
| First public | Nov 2025 proposal; spec Jan 26, 2026 [12][13] | Examples repo Oct 6, 2025; ChatKit repo Oct 4, 2025 [38] | Repo May 7, 2025 [38] | Repo Sep 24, 2025; announcement date unverified [38] | RSC `streamUI` path (now marked experimental); tool-part rendering in current AI SDK UI docs [43][44] |
...
|---|---|---|---|
| Manifest | `.claude-plugin/plugin.json`; bundles `skills/`, `agents/`, `hooks/`, `.mcp.json`, `.lsp.json`, `monitors/`, `bin/`, `settings.json` [55] | `.codex-plugin/plugin.json`; bundles `skills/`, `.app.json`, `.mcp.json`, agents, commands, `hooks.json` [64] | manifest file (name not verified); bundles prompts, MCP servers, commands, themes, hooks, sub-agents, skills [60] |
